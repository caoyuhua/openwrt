...
static struct {
	...
	{"KickoutMac",					Set_KickoutMac_Proc},
	...
	IN	PSTRING			arg)
{
	...
	PSTRING					file_w = NULL;
	CHAR temp[20] = {0};
	CHAR fileValue[1152] = {0};
	PSTRING  fileName = NULL;

	fileName = AP_MAC_PROFILE_PATH;
...
*/
	...
	{
		RtmpOSFileWrite(file_w, fileValue, strlen(fileValue));
...
#endif /* RTMP_RBUS_SUPPORT */
			break;
		case CMD_RTPRIV_IOCTL_GET_ENCRYPTION:
		{
			RT_CMD_AP_IOCTL_ENCRYPTION *ssid_encryption = pData;

			/*wpa+tkip*/
			if ((pAd->ApCfg.MBSSID[0].WepStatus == 4)&&(pAd->ApCfg.MBSSID[0].AuthMode == 4))
			{
				ssid_encryption->enabled = 1;//默认
				ssid_encryption->wpa_version = 1;
				ssid_encryption->group_ciphers = 0;//默认
				ssid_encryption->pair_ciphers = 4;
				ssid_encryption->auth_suites = 4;
				ssid_encryption->auth_algs = 1;
			}/*wpa+aes*/
			else if ((pAd->ApCfg.MBSSID[0].WepStatus == 6)&&(pAd->ApCfg.MBSSID[0].AuthMode == 4))
			{
				ssid_encryption->enabled = 1;//默认
				ssid_encryption->wpa_version = 1;
				ssid_encryption->group_ciphers = 0;//默认
				ssid_encryption->pair_ciphers = 64;
				ssid_encryption->auth_suites = 4;
				ssid_encryption->auth_algs = 1;
			}/*wpa+tkip/aes*/
			else if ((pAd->ApCfg.MBSSID[0].WepStatus == 8)&&(pAd->ApCfg.MBSSID[0].AuthMode == 4))
			{
				ssid_encryption->enabled = 1;//默认
				ssid_encryption->wpa_version = 1;
				ssid_encryption->group_ciphers = 0;//默认
				ssid_encryption->pair_ciphers = 128;
				ssid_encryption->auth_suites = 4;
				ssid_encryption->auth_algs = 1;
			}/*wpa2+tkip*/
			else if ((pAd->ApCfg.MBSSID[0].WepStatus == 4)&&(pAd->ApCfg.MBSSID[0].AuthMode == 7))
			{
				ssid_encryption->enabled = 1;//默认
				ssid_encryption->wpa_version = 2;
				ssid_encryption->group_ciphers = 0;//默认
				ssid_encryption->pair_ciphers = 4;
				ssid_encryption->auth_suites = 4;
				ssid_encryption->auth_algs = 1;
			}/*wpa2+aes*/
			else if ((pAd->ApCfg.MBSSID[0].WepStatus == 6)&&(pAd->ApCfg.MBSSID[0].AuthMode == 7))
			{
				ssid_encryption->enabled = 1;//默认
				ssid_encryption->wpa_version = 2;
				ssid_encryption->group_ciphers = 0;//默认
			...
			else if ((pAd->ApCfg.MBSSID[0].WepStatus == 8)&&(pAd->ApCfg.MBSSID[0].AuthMode == 7))
			{
				ssid_encryption->enabled = 1;//默认
				ssid_encryption->wpa_version = 2;
				ssid_encryption->group_ciphers = 0;//默认
			...
			else if ((pAd->ApCfg.MBSSID[0].WepStatus == 4)&&(pAd->ApCfg.MBSSID[0].AuthMode == 9))
			{
				ssid_encryption->enabled = 1;//默认
				ssid_encryption->wpa_version = 3;
				ssid_encryption->group_ciphers = 0;//默认
				ssid_encryption->pair_ciphers = 4;
				ssid_encryption->auth_suites = 4;
				ssid_encryption->auth_algs = 1;
			}/*wpa/wpa2+aes*/
			else if ((pAd->ApCfg.MBSSID[0].WepStatus == 6)&&(pAd->ApCfg.MBSSID[0].AuthMode == 9))
			{
				ssid_encryption->enabled = 1;//默认
				ssid_encryption->wpa_version = 3;
				ssid_encryption->group_ciphers = 0;//默认
			...
			else if ((pAd->ApCfg.MBSSID[0].WepStatus == 8)&&(pAd->ApCfg.MBSSID[0].AuthMode == 9))
			{
				ssid_encryption->enabled = 1;//默认
				ssid_encryption->wpa_version = 3;
				ssid_encryption->group_ciphers = 0;//默认
			...
			/*default:none*/
			else /*((pAd->ApCfg.MBSSID[0].WepStatus == 1)&&(pAd->ApCfg.MBSSID[0].AuthMode == 0))*/
			{
				ssid_encryption->enabled = 1;//默认
				ssid_encryption->wpa_version = 0;
				ssid_encryption->group_ciphers = 0;//默认
				ssid_encryption->pair_ciphers = 1;
				ssid_encryption->auth_suites = 1;
				ssid_encryption->auth_algs = 0;
			}
		}
			break;
