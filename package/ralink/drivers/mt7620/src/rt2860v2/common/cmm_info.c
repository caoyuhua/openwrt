	...
	IN	PSTRING			arg)
{
	...
	memcpy(cMac,arg,20);
	memcpy(cMacTemp, cMac, 20);
	char *token = rstrtok(cMac,temp);
	while(token)
	{
		AtoH(token, &Addr2[i], 1);
		i++;
		token = rstrtok(NULL,temp);
	}
	DBGPRINT(RT_DEBUG_TRACE, ("MacTableLookup:%02x:%02x:%02x:%02x:%02x:%02x", PRINT_MAC(Addr2)));
	...
	{				
		...
		pEntry = NULL;
		DBGPRINT(RT_DEBUG_TRACE, ("KickOut - Bssid does not match mactable\n"));
		success = TRUE;
	}
	else
	{
	...
	DBGPRINT(RT_DEBUG_TRACE, ("Set_KickoutMac_Proc::(kickout=%s)\n", cMacTemp));
	return success;
}
...
#else
#define	MAC_LINE_LEN	(1+19+6+6+6+6+10+10+10+6+6)
VOID RTMPIoctlGetMacTable(
	IN PRTMP_ADAPTER pAd, 
	IN RTMP_IOCTL_INPUT_STRUCT *wrq)
{
...
/*	msg = kmalloc(sizeof(CHAR)*(MAX_LEN_OF_MAC_TABLE*MAC_LINE_LEN), MEM_ALLOC_FLAG);*/
	...
	memset(msg, 0 ,MAX_LEN_OF_MAC_TABLE*MAC_LINE_LEN );
	sprintf(msg,"%s","\n");
	sprintf(msg+strlen(msg),"%-19s%-6s%-6s%-6s%-6s%-6s%-6s%-10s%-10s%-10s\n",
		"MAC", "AP",  "AID", "PSM", "AUTH", "CTxR", "LTxR","LDT", "RxB", "TxB");
	...
	for (i=0; i<MAX_LEN_OF_MAC_TABLE; i++)
	{
		...
		if (IS_ENTRY_CLIENT(pEntry) && (pEntry->Sst == SST_ASSOC))
		{
			...
			sprintf(msg+strlen(msg),"%02x:%02x:%02x:%02x:%02x:%02x  ",
				pEntry->Addr[0], pEntry->Addr[1], pEntry->Addr[2],
				pEntry->Addr[3], pEntry->Addr[4], pEntry->Addr[5]);
			sprintf(msg+strlen(msg),"%-6d", (int)pEntry->apidx);
			sprintf(msg+strlen(msg),"%-6d", (int)pEntry->Aid);
