...
/*				PCHAR pBssidStr; */
		...
		{
			ULONG txpower;
			RTMP_DRIVER_TXPOWER_GET(pAd, &txpower);
...
/*				struct iw_range range; */
			...
			*/
		    ...
		    break;
		case RTPRIV_IOCTL_GET_HWMODE:	/*定义SIOCGIWHWMODE参数，实现hwmode的获取 */
		{
			ULONG hwmode;
			RTMP_DRIVER_HWMODE_GET(pAd, &hwmode);
			wrqin->u.param.value = hwmode;
			wrqin->u.param.flags = 0;
		}
		    break;
		case RTPRIV_IOCTL_GET_ENCRYPTION:
		{
			RT_CMD_AP_IOCTL_ENCRYPTION *pSsid_encryption = wrqin->u.data.pointer;
			RT_CMD_AP_IOCTL_ENCRYPTION *encrypte = NULL;
			os_alloc_mem(NULL, (UCHAR **)&encrypte, sizeof(RT_CMD_AP_IOCTL_ENCRYPTION));
			if (encrypte == NULL)
			{
				DBGPRINT(RT_DEBUG_ERROR, ("%s: Allocate memory fail!!!\n", __FUNCTION__));
				break;
			...
			memset(encrypte, 0, sizeof(RT_CMD_AP_IOCTL_ENCRYPTION));
			Status = RTMP_AP_IoctlHandle(pAd, wrq, CMD_RTPRIV_IOCTL_GET_ENCRYPTION, 0, encrypte, 0);
			
			if (copy_to_user(pSsid_encryption, encrypte, sizeof(RT_CMD_AP_IOCTL_ENCRYPTION)))
				Status = -EFAULT;

			os_free_mem(NULL, encrypte);
		}
			break;
